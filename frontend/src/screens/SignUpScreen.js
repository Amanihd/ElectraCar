...
import useMockAuthStore from '../store/useMockAuthStore';

const SignUpScreen = () => {
  const navigation = useNavigation();
  const { setUser } = useMockAuthStore();

  const { values, errors, handleChange, validate } = useAuthForm(['firstName', 'lastName', 'email', 'password']);
